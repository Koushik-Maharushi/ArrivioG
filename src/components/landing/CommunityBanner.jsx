...
const CommunityBanner = () => {
  ...
  const spawnBubble = useCallback(() => {
    ...
    if (validPosition) {
        ...
        setTimeout(() => setBubbles(prev => prev.filter(b => b.id !== id)), 1500);
    }
  }, []);
  ...
  return (
    <section className="relative w-full h-[32rem] sm:h-[40rem] bg-[#EAE8E4] overflow-hidden flex items-center justify-center">
      ...
      <div className="absolute inset-0 opacity-[0.08] pointer-events-none mix-blend-multiply z-10" style={{ backgroundImage: `url("https://www.transparenttextures.com/patterns/stardust.png")` }}></div>
      ...
      <div className="z-30 text-center px-6 pointer-events-none relative">
         ...
         {/* === UPDATED LABEL STYLING (Matched from WhoWeServe) === */}
        ...
        <AnimatePresence>
            {bubbles.map((bubble) => (
                <motion.div
                    ...
                    style={{ top: `${bubble.top}%`, left: `${bubble.left}%`, position: 'absolute', x: "-50%", y: "-50%" }}
                    className={`
